class Route
  ...
  def matches?(req)
    return true if @http_method == req.request_method.downcase.to_sym && !!(@pattern =~ req.path)
    return false
  end
  ...
  def run(req, res)
    ...
    keys_vals = match_data.names.zip(match_data.captures)
    route_params = Hash[keys_vals]
